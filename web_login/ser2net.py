...
class Logger(object):
    ...
    @staticmethod
    def log_test(stream=sys.stdout, stream_level=logging.INFO, logfile=None, logfile_level=logging.DEBUG,
...
def send_cmd(cli, cmd='', timeout=30, check_prompt=False, check_ret=False,
    ...
    if not timeout:
        timeout = 30
    log.info(
        "Sending: %s, timeout=%s, prompt=%s, ret=%s, echo=%s" % (cmd, timeout, check_prompt, check_ret, expect_echo))
